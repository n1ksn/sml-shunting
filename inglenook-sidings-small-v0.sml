...
fun p2t1 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 = 1) andalso (length t1 >= 2)) then
...
fun p2t2 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 = 1) andalso (length t2 = 2)) then
    (t0 @ t2, t1, [], t3)
  else
    empty_state;
...
fun p2t3 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 = 1) andalso (length t3 = 2)) then
    (t0 @ t3, t1, t2, [])
  else
    empty_state;
...
fun d2t1 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 = 3) andalso (length t1 < 2)) then
    (List.take(t0, 1), tl(t0) @ t1, t2, t3)
  else
    empty_state;

fun d2t2 ((t0, t1, t2, t3):state):state =
  (* Drop two cars to track 2 *)
  if ((length t0 = 3) andalso (length t2 = 0)) then
    (List.take(t0, 1), t1, tl(t0), t3)
  else
    empty_state;

fun d2t3 ((t0, t1, t2, t3):state):state =
  (* Drop two cars to track 3 *)
  if ((length t0 = 3) andalso (length t3 = 0)) then
    (List.take(t0, 1), t1, t2, tl(t0))
  else
    empty_state;

fun p1t1 ((t0, t1, t2, t3):state):state =
  (* Pull one car from track 1 *)
  if ((length t0 < 3) andalso (length t1 >= 1)) then
...
fun p1t2 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 < 3) andalso (length t2 >= 1)) then
...
fun p1t3 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 < 3) andalso (length t3 >= 1)) then
...
fun d1t1 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 >= 2) andalso (length t1 < 3)) then
...
fun d1t2 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 >= 2) andalso (length t2 < 2)) then
    let val n = (length t0) - 1 in
...
fun d1t3 ((t0, t1, t2, t3):state):state =
  ...
  if ((length t0 >= 2) andalso (length t3 < 2)) then
    let val n = (length t0) - 1 in
...
fun solve (start:state, finish:state) =
  (* Find a minimal length solution path from start state to finish
     ...
     the solution *)
  print_solution (iddfs(start, finish, 20));

fun generate_problem (0) = print "\nDone!\n"
  | generate_problem (n) =
  let
    val perm = rnd_permu([1, 2, 3, 4, 5]);
    val start:state = ([0], List.take(perm, 3), List.drop(perm, 3), []);
    val finish:state = ([0], [1, 2, 3], [4, 5], [])
  in
